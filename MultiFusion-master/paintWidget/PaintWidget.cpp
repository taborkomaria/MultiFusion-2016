...
void PaintWidget::showRectImage(){
    ...
    if (!fileName.isEmpty()){
        ...
        trans = trans.rotate(23);
        //img = img.transformed(trans);
        /*QString str = QString::number(imageWidth);
        QMessageBox::information(NULL,QObject::tr("Информация"),str);*/
        painter.drawRectImage(imageHeight, imageWidth,img);
        if(painter.selection.countSelected() > 0)
        {
            QPen pen;
            pen.setWidth(0);
            pen.setBrush(Qt::transparent);
            painter.selection.setPen(pen);
            //painter.selection.setBrush( img );
            painter.update();
        }
        else{
            QMessageBox::information(NULL,QObject::tr("Информация"),tr("Нет выделенных объектов"));
        }
    }
    else{
