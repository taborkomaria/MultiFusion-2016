...
/**
*	@file gobject.h Данный файл содержит описание класса, являющегося базовым абстрактным классом для
*			всех классов, которыя являются графическими обьектами.
*/

/**
*		Класс, являющийся базовым абстрактным классом для
*	всех классов, которыя являются графическими обьектами.
*/
class GObject:public GObjectInterface
{
	Q_OBJECT
	Q_INTERFACES(GObjectInterface)
	public:
    virtual int rewriteLastPointGuideCoord( const QPoint &p ) = 0;
            virtual void deleteLastGuide() = 0;
            virtual void reestablishLastGuid() = 0;

    /**
     * Удаляет все кадры кроме frame
     * @param frame, если равен -1 удаляет абсолютно все кадры
     */
   // virtual bool isImageFigure() = 0;
		...
		*
		*	@param position номер удаляемого кадра.
		*/
    ...
    */
    virtual void flip( qreal sx, qreal sy, const QPointF &scaleCenter ) = 0;
		/**
		*	Производит масштабирование обьекта.
		*
		*	@param sx коэффициент масштабирования по оси X.
		...
		*/
		static GObjectInterface *create( QObject* figureTool, const QPointF &pos,
									const QString &name, int position );
...
template < class T > inline GObject* GOBJECT( T *o )
{
	return qobject_cast< GObject* >( o );
}
