...
class GProperties
{
	public:
		...
		GProperties &operator=( const GProperties &p )
		{
            ...
            brushImage = p.brushImage;
            _scaledHeight = p._scaledHeight;
            _scaledWidth = p._scaledWidth;
            rotateAngle = p.rotateAngle;
            _transformed = p._transformed;
			return ( *this );
		}
		...
		bool			isTransform;
		QPolygonF 		points;
        /* изображение в текущем кадре */
        QPixmap         brushImage;
        /* угол поворота изображения в текущем кадре */
        qreal           rotateAngle;
        /* ширина изображения в текущем кадре */
        int             _scaledWidth;
        /* высота изображения в текущем кадре */
        int             _scaledHeight;
        /* было ли искажение изображения в текущем кадре */
        bool            _transformed;
};
