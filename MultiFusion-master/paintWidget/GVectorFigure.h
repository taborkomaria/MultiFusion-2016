...
class GVectorFigure:public GObject
{
	...
	public:
    virtual int rewriteLastPointGuideCoord( const QPoint &p );
    virtual void deleteLastGuide();
    virtual void reestablishLastGuid();

        /**
        * Запоминает оригинальное изображение
        * при загрузке изображения
        */
        virtual void setOriginalImage(QPixmap img);
    ...
    *	@param scaleCenter точка, относительно которой производится масштаирование.
    */
    virtual void flip( qreal sx, qreal sy, const QPointF &scaleCenter );
		...
		virtual void paint( QPainter &p );
				...
				*/
                GVectorFigure( const QPolygonF &points = QPolygonF(),
                                bool _spline = false, bool _closed = false, int position = 0, bool image = false,
                               bool scaled = false);
	...
	private:
        ...
        /* свойство, которое определяет является ли фигура изображением */
        bool imageFigure;
        /* свойство, которое определяет было ли искажение изображения */
        bool beenScaled;
        /* свойство, которое хранит оригинальное изображение */
