...
void RealPaintWidget::mouseMoveEvent( QMouseEvent * event )
{
    ...
    if( ( cont->countObjects()==1 ) && ( currentTool != 0 ) && ( currentTool->createStyle() == FigureToolInterface::paintAfterRelease ) )
    {
        GObjectInterface *o = layers[currentLayer]->object( 0 );
        if( o == 0 ) return;
        if ( ( isMousePress == 1 ) && (  QString::compare(currentTool->figureName(), "Bezier line", Qt::CaseInsensitive ) == 0 ) )
        {
...
void RealPaintWidget::drawRectImage(int imageHeight, int imageWidth, QPixmap img){
    ...
    if (!fileName.isEmpty()){
        ...
        //QString strImage;
        //strImage.setNum(imageHeight);
       // QMessageBox::information(this, "Caption", strImage);
        ...
        /*GObjectInterface * o = GObject::create( currentTool, pos,
            ...
            GVectorFigure *o = new GVectorFigure( points, false, true, currentFrame,true);
            o->setOriginalImage(img);
            o->setFrame(currentFrame);
            o->move( 0, 0 );
            //QMessageBox::warning(WIDGET(f->parent()),"Trace",QString::number(pos.x()));
            o->setObjectName( "pic" );
            o->scale(imageWidth,imageHeight,QPointF(0,0));
    ...
    /*}
        ...
        QMessageBox::information(NULL,QObject::tr("Информация"),tr("Не выбран файл"));
    }*/
}
void RealPaintWidget::mousePressEvent( QMouseEvent *event )
{
 ...
 if(button == Qt::LeftButton || selection.isInAddPointMode())
 {
  ...
  if( currentTool != 0 )
  {
            ...
            if(cont->countObjects()==1 && currentTool->createStyle() == FigureToolInterface::paintAfterRelease && selection.getIsNewFigure() )
            {
                cont->reestablishLastGuid();
   ...
   selection.setIsNewFigure(true);
            ...
            if( currentTool->createStyle() == FigureToolInterface::paintAfterRelease )
            {
   ...
   emit figureSelected( currentLayer, layers[currentLayer]->objectIndex( GOBJECT(o) ) );
            ...
            if( currentTool->createStyle() == FigureToolInterface::createAndResize )
   {
    selection.mousePress( event->button(), pos, event->modifiers() );
    selection.setCreateFigureMode( true );
   }
 ...
 if( inSelectionMode )
 {
  inSelectionMode = false;
   ...
   return;
    ...
    if( currentTool != 0 && currentTool->createStyle() == FigureToolInterface::paintAfterRelease )
    {
        isMousePress = false;
        GObjectInterface *s = layers[currentLayer]->object( 0 );
        if( s != 0 )
        {
            ...
            QPolygonF points = s->points(currentFrame);
            if( points.size() > 4 )
            {
                QPointF p = points[ points.size() - 1 ] - points[ 0 ];
                if( ( fabs( p.x() ) < 10 ) && ( fabs( p.y() ) < 10 ) )
                {
                    QMessageBox msgBox;
                    msgBox.setInformativeText("Замкнуть контур и закончить рисование?");
                    if (msgBox.exec() == 1024)
                    {
                        points[ 0 ] = points[ points.size() - 1 ];
                        s->setClosed(true);
                        selection.setIsNewFigure(false);
                        update();
                        return;
                    }
                }
