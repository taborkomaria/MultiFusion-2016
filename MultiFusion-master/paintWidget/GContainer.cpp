...
#include <QMessageBox>
void GContainer::reestablishLastGuid()
{
objects[0]->reestablishLastGuid();
}
...
void GContainer::flip( qreal sx, qreal sy, const QPointF &scaleCenter )
{
    if( !isEditable() )
        return;
    ...
    int countObjects = objects.size();
    for( int i = 0; i < countObjects; i++ )
        objects[i]->flip( sx, sy, scaleCenter );
}
