...
/**TODO Дописать функции */
 ...
 void GSelectionRect::setScale(qreal dx, qreal dy)
 {
     ...
     int countObjects = selectedObjects->countObjects();
     bool imageInside = false;
     /*for( int i = 0; i < countObjects; i++ ){
         GObject *obj = selectedObjects->object(i);
         if(obj->isImageFigure()){
             //QString str = QString::number(countObjects);
